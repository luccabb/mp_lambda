        ...
        if piece:
            ...
            if piece.symbol.lower() == 'p':
                ...
                for j in range(8):
                    ...
                    if piece_helper.symbol.lower() == 'p' and piece_helper.color == piece_color:
                        pawn_in_col += 1
                
                if pawn_in_col >= 2:
                    if piece_color == chess.BLACK:
                        total_value -= 0.5*pawn_in_row
                    else:
                        total_value += 0.5*pawn_in_row
...
def get_move_score(board, depth, player: bool):
    ...
    return [best_move_value, best_move if best_move else random.choice(move)]
...
def lambda_handler(event, context):
    fen = event['queryStringParameters']['fen']
    ...
    result = pool.starmap(get_move_score, arguments)
